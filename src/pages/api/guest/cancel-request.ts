...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (RESEND_API_KEY) {
    try {
      const hotelEmail = await db?.prepare('SELECT email, contact_email FROM hotels WHERE id = ?').bind(booking.hotel_id).first() as any;
      const hotelTo = [...new Set([hotelEmail?.email, hotelEmail?.contact_email].filter(Boolean))];
      if (hotelTo.length > 0) {
