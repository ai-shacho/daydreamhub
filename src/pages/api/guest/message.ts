    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family:Arial,sans-serif;max-width:600px"><h3>New message from guest for Booking #${booking_id}</h3><p><strong>From:</strong> ${auth.name || auth.email}</p><p><strong>Message:</strong></p><blockquote style="border-left:3px solid #46a3c2;padding:8px 16px;color:#374151">${content.trim()}</blockquote><a href="${runtime?.env?.SITE_URL || 'https://daydreamhub.com'}/owner/bookings" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#46a3c2;color:white;text-decoration:none;border-radius:6px">Reply in Owner Portal</a></div>`,
        }),
      });
    } catch {}
  }
